...
#def test_user_testcase():
#    auth_token = os.environ.get('BOPTEST_DASHBOARD_API_KEY')
#    assert(auth_token)
#
#    testcase_namespace = os.environ.get('BOPTEST_DASHBOARD_USERNAME')
#    assert(testcase_namespace)
#
#    testcase_id = 'testcase1'
#    testcase_path = f'/testcases/{testcase_id}/models/wrapped.fmu'
#
#    response = get_testcase_form(testcase_id, testcase_namespace, auth_token + 'xyz')
#    assert response.status_code == 401
#
#    response = get_testcase_form(testcase_id, testcase_namespace, auth_token)
#    assert response.status_code == 200
#
#    response = upload_testcase(response, testcase_path)
#    assert response.status_code == 204
#
#    response = confirm_testcase(testcase_id)
#    assert response.status_code == 200
