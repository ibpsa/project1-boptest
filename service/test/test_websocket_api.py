...
@pytest.mark.asyncio
async def test_restock_model():
    async with websockets.connect(ws_host) as websocket:
        auth_token = os.environ.get("BOPTEST_TEST_PRIVILEGED_KEY")
        testcase_id = "wrapped"
        testcase_path = f"/boptest/testcases/{testcase_id}.fmu"
        ...
        # Send a message to advance with furnace on
        requestid = str(uuid.uuid4())
        u = {'overwrite_FurnaceStatus_u': '1', 'overwrite_FurnaceStatus_activate': '1'}
        request = {"requestid": requestid, "testid": testid, "method": "advance", "params": u}
        await websocket.send(json.dumps(request))
        ...
        check.greater(payload['read_FurnaceHeat_y'], 10.0)

        # Send a message to advance with no control signals
        requestid = str(uuid.uuid4())
        request = {"requestid": requestid, "testid": testid, "method": "advance", "params": {}}
        await websocket.send(json.dumps(request))
        ...
        # Furnace should still be on
        responsedata = response.get("responsedata")
        check.is_true(responsedata)
        payload = responsedata['payload']
        check.greater(payload['read_FurnaceHeat_y'], 10.0)

        # Send a message to advance with furnace off
        # Note that numerical values are sent as strings. This is an artifact of boptest's testcase.py library
        requestid = str(uuid.uuid4())
        u = {'overwrite_FurnaceStatus_u': '0', 'overwrite_FurnaceStatus_activate': '1'}
        ...
        check.is_true(responsedata)
        payload = responsedata['payload']
        check.almost_equal(payload['read_FurnaceHeat_y'], 0.0)
