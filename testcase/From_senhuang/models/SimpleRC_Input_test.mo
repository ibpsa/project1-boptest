...
model SimpleRC_Input_test
  ...
  Modelica.Blocks.Continuous.Integrator intEHeat(initType=Modelica.Blocks.Types.Init.InitialState,
   ...
   inner IBPSA.Utilities.IO.RESTClient.Configuration config(activation=IBPSA.Utilities.IO.RESTClient.Types.GlobalActivation.always,
      samplePeriod=60)
    annotation (Placement(transformation(extent={{80,80},{100,100}})));
  IBPSA.Utilities.IO.RESTClient.Read_Real read_Real(
  ...
  IBPSA.Utilities.IO.RESTClient.Send_Real send_Real(
    hostAddress="127.0.0.1",
    tcpPort=8888,
    numVar=2,
    varName={"time","tzone"})
...
equation
  ...
  connect(preTOut.port, res.port_a)
    annotation (Line(points={{-30,0},{-20,0}}, color={191,0,0}));
  connect(preHeat.port, cap.port)
    annotation (Line(points={{20,-52},{40,-52},{40,-26},{20,-26},{20,0}},
                                                         color={191,0,0}));
  connect(eff.y, PHeat) annotation (Line(points={{-1,-80},{40,-80},{40,-60},{110,
          -60}},     color={0,0,127}));
  connect(intEHeat.y,EHeat)
    annotation (Line(points={{89,-80},{110,-80}}, color={0,0,127}));
  connect(intEHeat.u, PHeat) annotation (Line(points={{66,-80},{40,-80},{40,-70},
          {40,-60},{110,-60}},          color={0,0,127}));
  connect(eff.u, preHeat.Q_flow) annotation (Line(points={{-24,-80},{-80,-80},{
          -80,-52},{0,-52}}, color={0,0,127}));
  connect(preTOut.T, souTOut.y) annotation (Line(points={{-52,0},{-70,0},{-70,
          40},{-79,40}}, color={0,0,127}));
  connect(realExpression.y, read_Real.u[2]) annotation (Line(points={{-79,-20},
          {-74,-20},{-74,-29},{-52,-29}}, color={0,0,127}));
  connect(realExpression1.y, read_Real.u[1]) annotation (Line(points={{9,80},{
          34,80},{34,62},{-60,62},{-60,-31},{-52,-31}}, color={0,0,127}));
  connect(read_Real.y[2], preHeat.Q_flow) annotation (Line(points={{-28.6,-29.3},
          {-20,-29.3},{-20,-52},{0,-52}}, color={0,0,127}));
  connect(send_Real.u[1], realExpression1.y) annotation (Line(points={{78,39},{
          54,39},{54,80},{9,80}}, color={0,0,127}));
  connect(send_Real.u[2], senTZone.T)
    annotation (Line(points={{78,41},{64,41},{64,0},{60,0}}, color={0,0,127}));
  annotation (uses(Modelica(version="3.2.2"), IBPSA(version="2.0.0")),
      experiment(StopTime=86400));
end SimpleRC_Input_test;
