...
model AirSideBackup "\"air sidesystem\""
  ...
  parameter Modelica.Units.SI.Efficiency eps5(max=1) = 0.8
    "Heat exchanger effectiveness of vav 1";
  BuildingControlEmulator.Systems.Floor botFlo(
    ...
    duaFanAirHanUnit(
      ...
      Fan_k=0.001,
      Fan_Ti=600,
      booleanExpression(y=botFlo.duaFanAirHanUnit.On)),
  ...
  BuildingControlEmulator.Systems.Floor midFlo(
    redeclare package MediumAir = MediumAir,
    redeclare package MediumHeaWat = MediumHeaWat,
    PreDroCoiAir=PreDroCoiAir,
    PreDroMixingBoxAir=PreDroMixingBoxAir,
    ...
    duaFanAirHanUnit(
      ...
      Fan_k=0.001,
      Fan_Ti=600,
      booleanExpression(y=midFlo.duaFanAirHanUnit.On)),
    redeclare package MediumCooWat = MediumCHW,
    fivZonVAV(vol(V=200000), each vAV(Dam(riseTime=15))))
    annotation (Placement(transformation(extent={{114,20},{164,62}})));

  BuildingControlEmulator.Systems.Floor topFlo(
    redeclare package MediumAir = MediumAir,
    redeclare package MediumHeaWat = MediumHeaWat,
    PreDroCoiAir=PreDroCoiAir,
    PreDroMixingBoxAir=PreDroMixingBoxAir,
    ...
    duaFanAirHanUnit(
      ...
      Fan_k=0.001,
      Fan_Ti=600,
      booleanExpression(y=topFlo.duaFanAirHanUnit.On)),
    redeclare package MediumCooWat = MediumCHW,
  ...
  Buildings.Utilities.IO.SignalExchange.Overwrite oveBotFloTSupAir
    annotation (Placement(transformation(extent={{-30,46},{-10,66}})));
  Buildings.Utilities.IO.SignalExchange.Overwrite oveMidFloTSupAir
    annotation (Placement(transformation(extent={{-30,46},{-10,66}})));
  Buildings.Utilities.IO.SignalExchange.Overwrite oveTopFloTSupAir
...
equation
   ...
   annotation (Line(points={{38,40},{-38,40},{-38,0},{-114,0}},
                          color={0,0,127}));
  connect(botFlo.port_Exh_Air, sou[1].ports[1]) annotation (Line(
      points={{114,32.6},{90,32.6},{90,38.6667},{60,38.6667}},
      color={28,108,200},
      thickness=0.5));
  connect(botFlo.port_Fre_Air, sou[1].ports[2]) annotation (Line(
      points={{114,49.4},{90,49.4},{90,36},{60,36}},
      color={28,108,200},
      thickness=0.5));
  connect(dpStaSet[1].y, botFlo.PreSetPoi) annotation (Line(points={{-49,20},{102,
          20},{102,43.1},{111.5,43.1}}, color={0,0,127}));
  connect(oveBotFloTSupAir.y, botFlo.DisTemPSetPoi) annotation (Line(points={{-9,
          56},{100,56},{100,53.6},{111.5,53.6}}, color={0,0,127}));
  connect(reaToBooOcc.y, botFlo.OnFan) annotation (Line(points={{-39,100},{-36,100},
          {-36,74},{106,74},{106,30.5},{111.5,30.5}}, color={255,0,255}));
  connect(botFlo.OnZon, onZon[1].y) annotation (Line(points={{111.5,22.1},{108,22.1},
          {108,22},{104,22},{104,76},{-49,76}}, color={255,0,255}));
   for j in 1:5 loop
    connect(botFlo.TZon[j], zonVAVCon[(1 - 1)*5 + j].T) annotation (Line(points=
           {{166.5,41},{166.5,42},{180,42},{180,80},{44,80},{44,100},{50,100}},
          color={0,0,127}));
    connect(zonVAVCon[(1 - 1)*5 + j].yAirFlowSetPoi, botFlo.AirFlowRatSetPoi[j])
      annotation (Line(points={{73.1,106.1},{100,106.1},{100,38.9},{111.5,38.9}},
          color={0,0,127}));
    connect(zonVAVCon[(1 - 1)*5 + j].yValPos, botFlo.yVal[j]) annotation (Line(
          points={{73.1,94.1},{102,94.1},{102,26.3},{111.5,26.3}}, color={0,0,127}));
    connect(loa[(1 - 1)*5 + j], botFlo.Q_flow[j]);
    connect(botFlo.TZon[j], TZon[(1 - 1)*5 + j]);
    ...
    connect(TZonAirSet[(1 - 1)*5 + j].SetPoi[2], zonVAVCon[(1 - 1)*5 + j].THeaSetPoi)
      annotation (Line(points={{22,101},{22,100},{34,100},{34,94},{50,94}},
          color={0,0,127}));
    connect(TZonAirSet[(1 - 1)*5 + j].SetPoi[1], botFlo.ZonCooTempSetPoi[j])
      annotation (Line(points={{22,99},{32,99},{32,62},{111.5,62}}, color={0,0,127}));
    connect(TZonAirSet[(1 - 1)*5 + j].SetPoi[2], botFlo.ZonHeaTempSetPoi[j])
      annotation (Line(points={{22,101},{30,101},{30,57.8},{111.5,57.8}}, color=
           {0,0,127}));
   end for;

   for i in 1:5 loop
    connect(loa[(2 - 1)*5 + i], loaMulMidFlo[i].u) annotation (Line(points={{-114,60},
            {-95.2,60}}, color={0,0,127}));
   end for;
   connect(sou[2].T_in, TDryBul);
  connect(midFlo.port_Exh_Air, sou[2].ports[1]);
  connect(midFlo.port_Fre_Air, sou[2].ports[2]);
  connect(dpStaSet[2].y, midFlo.PreSetPoi);
  connect(oveMidFloTSupAir.y, midFlo.DisTemPSetPoi);
  connect(reaToBooOcc.y, midFlo.OnFan);
  connect(midFlo.OnZon, onZon[2].y);
   for j in 1:5 loop
    connect(midFlo.TZon[j], zonVAVCon[(2 - 1)*5 + j].T);
    connect(zonVAVCon[(2 - 1)*5 + j].yAirFlowSetPoi, midFlo.AirFlowRatSetPoi[j]);
    connect(zonVAVCon[(2 - 1)*5 + j].yValPos, midFlo.yVal[j]);
    connect(loaMulMidFlo[j].y, midFlo.Q_flow[j]) annotation (Line(points={{-81.4,
            60},{-76,60},{-76,2},{142,2},{142,17.9},{141.5,17.9}}, color={0,0,127}));
    connect(midFlo.TZon[j], TZon[(2 - 1)*5 + j]);
    ...
    connect(TZonAirSet[(2 - 1)*5 + j].SetPoi[1], midFlo.ZonCooTempSetPoi[j]);
    connect(TZonAirSet[(2 - 1)*5 + j].SetPoi[2], midFlo.ZonHeaTempSetPoi[j]);
   end for;

   connect(sou[3].T_in, TDryBul);
  connect(topFlo.port_Exh_Air, sou[3].ports[1]);
  connect(topFlo.port_Fre_Air, sou[3].ports[2]);
  connect(dpStaSet[3].y, topFlo.PreSetPoi);
  connect(oveTopFloTSupAir.y, topFlo.DisTemPSetPoi);
  connect(reaToBooOcc.y, topFlo.OnFan);
  connect(topFlo.OnZon, onZon[3].y);
   for j in 1:5 loop
    connect(topFlo.TZon[j], zonVAVCon[(3 - 1)*5 + j].T);
    connect(zonVAVCon[(3 - 1)*5 + j].yAirFlowSetPoi, topFlo.AirFlowRatSetPoi[j]);
    connect(zonVAVCon[(3 - 1)*5 + j].yValPos, topFlo.yVal[j]);
    connect(loa[(3 - 1)*5 + j], topFlo.Q_flow[j]);
    connect(topFlo.TZon[j], TZon[(3 - 1)*5 + j]);
    ...
    connect(TZonAirSet[(3 - 1)*5 + j].SetPoi[1], topFlo.ZonCooTempSetPoi[j]);
    connect(TZonAirSet[(3 - 1)*5 + j].SetPoi[2], topFlo.ZonHeaTempSetPoi[j]);
   end for;
  connect(booRep.y, TZonAirSet.Occ)
    annotation (Line(points={{-9,100},{-2,100}},
                                               color={255,0,255}));
  connect(botFlo.TOut, TDryBul) annotation (Line(points={{134,17.9},{134,0},{-84,
          0},{-84,0},{-114,0}}, color={0,0,127}));
  connect(midFlo.TOut, TDryBul);
  connect(topFlo.TOut, TDryBul);
  connect(TSupAirSet[1].y, oveBotFloTSupAir.u)
    annotation (Line(points={{-49,56},{-32,56}}, color={0,0,127}));
  connect(TSupAirSet[2].y, oveMidFloTSupAir.u);
  connect(TSupAirSet[3].y, oveTopFloTSupAir.u);
