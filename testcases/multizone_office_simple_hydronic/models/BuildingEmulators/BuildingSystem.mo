...
model BuildingSystem
  ...
  inner .IDEAS.BoundaryConditions.SimInfoManager sim(filNam = Modelica.Utilities.Files.loadResource("/home/iago/Documents/DeltaQ/DQ_DevEnv/Repositories/IDEAS/IDEAS/Resources/weatherdata/BEL_VLG_Uccle.064470_TMYx.2007-2021.mos"), unify_n50 = false)
    "Simulation information manager for climate data"
    annotation (Placement(transformation(extent={{-80,80},{-60,100}})));

  package Medium = .IDEAS.Media.Air(extraPropertiesNames={"CO2"}) "Air medium";

  .BuildingEmulators.Templates.Structure.TwoZoneKPI structure
  ...
  .BuildingEmulators.Templates.Ventilation.VentilationSystemWithCAVsPerZoneKPI ventilation(
    ...
    dp_nominal_air_sup(each displayUnit="Pa") = 500 * ones(structure.nZones),
    dp_nominal_air_ret(each displayUnit="Pa") = 450 * ones(structure.nZones))
    ...
    .BuildingEmulators.Templates.Occupancy.OccupantNumber occupancy(
        nZones = structure.nZones,
        maxOcc = sum(structure.AZones) / 15,
        use_heaPor = false) annotation(Placement(transformation(extent = {{11.053571428571416,-34.24402573529413},{-28.946428571428584,-14.244025735294127}},origin = {0.0,0.0},rotation = 0.0)));
    .BuildingEmulators.Templates.Heating.FanCoilUnitsKPI heating_cooling(
        nZones = structure.nZones,
        Q_design = {50600,50600},
        dp_nominal_wat_emi = 3e5,
        hp_nominal_wat_emi = 3e5,
        dp_nominal_wat_pro = 2e5,
        hp_nominal_wat_pro = 2e5,
