...
model FanCoilUnits
  ...
  parameter Modelica.Units.SI.HeatFlowRate QCoo_flow_nominal = sum(QCooEmi_flow_nominal) + sum(QCooAhu_flow_nominal) "Nominal heat transfer of the cooling production system" annotation (
    Dialog(group = "Nominal power"));
  parameter Modelica.Units.SI.HeatFlowRate QHeaEmi_flow_nominal[nZones] = Q_design "Nominal heat transfer of the FCU heating coil" annotation (
    Dialog(group = "Nominal power"));
  parameter Modelica.Units.SI.HeatFlowRate QCooEmi_flow_nominal[nZones] = -Q_design "Nominal heat transfer of the FCU cooling coil" annotation (
    Dialog(group = "Nominal power"));
  ...
  parameter .Modelica.Units.SI.PressureDifference dp_nominal_wat_emi(min=0)
    "Emission nominal pressure drop" annotation (Dialog(group="Nominal condition"));
  parameter .Modelica.Units.SI.PressureDifference hp_nominal_wat_emi(min=0)
    "Emission nominal pressure head" annotation (Dialog(group="Nominal condition"));
  parameter .Modelica.Units.SI.PressureDifference dp_nominal_wat_pro(min=0)
    "Production nominal pressure drop" annotation (Dialog(group="Nominal condition"));
  parameter .Modelica.Units.SI.PressureDifference hp_nominal_wat_pro(min=0)
    "Production nominal pressure head" annotation (Dialog(group="Nominal condition"));

  parameter .Modelica.Units.SI.MassFlowRate[nZones] mAir_flow_nominal = Q_design/1004/10 "Nominal mass flow rate of air";
  ...
  parameter Modelica.Units.SI.MassFlowRate[nZones] mWatEmiHea_flow_nominal = Q_design / 4180 / deltaTHea_nominal "Nominal mass flow of the heating coil";
  ...
  IDEAS.Fluid.Sources.Boundary_pT bouCoo(
    ...
    .IDEAS.Fluid.Sensors.TemperatureTwoPort senTSupProCoo(tau = 0,redeclare package Medium = MediumCooling,m_flow_nominal = 1,allowFlowReversal = false) annotation(Placement(transformation(extent = {{-22.567980396864016,-42.567980396864016},{-33.432019603135984,-53.432019603135984}},origin = {0.0,0.0},rotation = 0.0)));
    .BuildingEmulators.Components.MixingStage[nVen] mixingAhuHea(each dp_nominal = dp_nominal_wat_emi,m_flow_nominal_emi = mWatAhuHea_flow_nominal) annotation(Placement(transformation(extent = {{10.0,-10.0},{-10.0,10.0}},origin = {143.55261249999992,60.65324852941174},rotation = -90.0)));
    .BuildingEmulators.Components.MixingStage[nVen] mixingAhuCoo(each dp_nominal = dp_nominal_wat_emi,m_flow_nominal_emi = mWatAhuCoo_flow_nominal) annotation(Placement(transformation(extent = {{10.0,-10.0},{-10.0,10.0}},origin = {59.13477794117637,53.03643333333331},rotation = -90.0)));
    ...
    .IDEAS.Fluid.FixedResistances.PressureDrop preDroProHea(redeclare package Medium = MediumHeating,allowFlowReversal = false,m_flow_nominal = mWatProHea_flow_nominal,dp_nominal = dp_nominal_wat_pro) "Flow resistance to decouple pressure state from boundary" annotation(Placement(transformation(extent = {{95.19929311378094,-7.6303412349598805},{84.80070688621906,3.6303412349598805}},rotation = 0.0,origin = {0.0,0.0})));
    ...
    .IDEAS.Fluid.Sensors.TemperatureTwoPort senTRetProCoo(allowFlowReversal = false,m_flow_nominal = 1,redeclare package Medium = MediumCooling,tau = 0) annotation(Placement(transformation(extent = {{-39.432019603135984,-64.56798039686402},{-28.567980396864016,-75.43201960313598}},origin = {0.0,0.0},rotation = 0.0)));
    .IDEAS.Fluid.Sensors.TemperatureTwoPort senTRetProHea(tau = 0,redeclare package Medium = MediumHeating,m_flow_nominal = 1,allowFlowReversal = false) annotation(Placement(transformation(extent = {{46.567980396864016,-24.567980396864016},{57.432019603135984,-35.432019603135984}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
  ...
  for i in 1:nZones loop
    ...
    connect(zon[i].ports[1], fanCoilUnit[i].port_air_a) annotation (Line(points={{-128,29},
            {-130,29},{-130,34},{-114,34},{-114,102},{-74.8,102},{-74.8,96}},
                                                             color={0,127,255}));
    connect(fanCoilUnit[i].port_air_b, zon[i].ports[2]) annotation (Line(points={{-57.9,
