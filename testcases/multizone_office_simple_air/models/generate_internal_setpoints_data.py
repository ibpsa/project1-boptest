...
df = pd.read_csv('internal_setpoints_occupancy_sim.csv',index_col='Time')
...
for key in df.columns:
    if 'InternalGainsLat' in key:
        ...
        df['Occupancy[{0}]'.format(zone)] = (df['InternalGainsLat[{0}]'.format(zone)].values/lat_per).astype(int)
