...
def run(plot=False, customized_kpi_config=None):
    ...
    testid = client.submit('../../testcases/testcase1/models/wrapped.fmu')
    # ---------------
    ...
    # TODO step resource is does not return a valid value until after the test is initialized
    ...
    res = requests.put('{0}/step/{1}'.format(url,testid), json={'step':step})
    print('\nRunning test case...')
    # Initialize u
    u = pid.initialize()
    # Simulation Loop
...
if __name__ == "__main__":
    ...
    run(customized_kpi_config='custom_kpi/custom_kpis_example.config')
