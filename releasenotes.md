...
- Use Python arrays, with 32 bit single float precision, instead of numpy arrays for storage of data in ``y_store`` and ``u_store`` in ``testcase.py`` for more efficient memory utilization.  This is for [#520](https://github.com/ibpsa/project1-boptest/issues/520).
